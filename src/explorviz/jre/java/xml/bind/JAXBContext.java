...
package explorviz.jre.java.xml.bind;
 ...
 * Edition, Section 7.4.1. Package Annotations).</li>
 * <li>{@link #newInstance(Class...) JAXBContext.newInstance(
 * com.acme.foo.Foo.class )} <br/>
 * The JAXBContext instance is intialized with class(es) passed as parameter(s)
 * and classes that are statically reachable from these class(es). See
 * {@link #newInstance(Class...)} for details.</li>
 * </ul>
 *
 * <p>
 * <blockquote> <i><B>SPEC REQUIREMENT:</B> the provider must supply an
 * implementation class containing the following method signatures:</i>
 *
 * <pre>
 ...
 * datatype converter that will be used during these
 * operations.</i> </blockquote>
 *
 * <p>
 * <a name="Unmarshalling"></a> <b>Unmarshalling</b>
 * <p>
 * <blockquote> The {@link Unmarshaller} class provides the client application
 * the ability to convert XML data into a tree of Java content objects. The
 * unmarshal method allows for any global XML element declared in the schema to
 * be unmarshalled as the root of an instance document. Additionally, the
 * unmarshal method allows for an unrecognized root element that has an xsi:type
 * attribute's value that references a type definition declared in the schema to
 * be unmarshalled as the root of an instance document. The <tt>JAXBContext</tt>
 ...
 * <tt>newInstance( javaContentInterface )</tt> method</i> </blockquote>
 *
 * <p>
 * <b>Marshalling</b>
 * <p>
 * <blockquote> The {@link Marshaller} class provides the client application the
 * ability to convert a Java content tree back into XML data. There is no
 * difference between marshalling a content tree that is created manually using
 * the factory methods and marshalling a content tree that is the result an
 * <tt>unmarshal
 ...
 * <b>Validation</b>
 * <p>
 * <blockquote> Validation has been changed significantly since JAXB 1.0. The
 ...
 * <blockquote> The following JAXB 1.0 restriction only applies to binding
 * schema to interfaces/implementation classes. Since this binding does not
 * require a common runtime system, a JAXB client application must not attempt
 * to mix runtime objects (<tt>JAXBContext,
 ...
 * @see Marshaller
 * @see Unmarshaller
 * @see <a href="http://java.sun.com/docs/books/jls">S 7.4.1.1
 *      "Package Annotations" in Java Language Specification, 3rd Edition</a>
 * @since JAXB1.0
 */
public abstract class JAXBContext {
	 ...
	 * {@link #newInstance(String,ClassLoader) newInstance} method. It uses the
	 * context class loader of the current thread. To specify the use of a
	 * different class loader, either set it via the
	 ...
	 * package annotations that must be processed. (see JLS 3rd Edition, Section
	 * 7.4.1. "Package Annotations").
	 * </p>
	 * 
	 * <p>
	 ...
	 * <tt><jaxb:globalBindings valueClass="false"></tt>, the JAXB provider will
	 * ensure that each package on the context path has a
	 ...
	 *             </ol>
	 */
	public static JAXBContext newInstance(final String contextPath, final ClassLoader classLoader)
			throws JAXBException {
	 ...
	 * instanciation of {@link JAXBContext}.
	 *
	 * <p>
	 ...
	 * optional package annotations exist, they must be processed. (see JLS 3rd
	 ...
	 * @since JAXB2.0
	 */
	public static JAXBContext newInstance(final Class[] classesToBeBound,
			final Map<String, ?> properties) throws JAXBException {
