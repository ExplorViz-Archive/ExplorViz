...
class ApplicationClientSide extends DrawNodeEntity implements IViewable {
    ...
    def createApplicationShape(Quad quad, float z) {
        if (database) {
    ...
    def private createApplicationDatabase(Quad quad, float z) {
    ...
    def private createApplicationBox(Quad quad, float z) {
        ...
        if (oldQuad == null) {
            if (image != null && !image.empty) {
                if (database) {
                    ...
                    texture = TextureManager::createTextureFromImagePath(image, 50, 50, 412, 156, 512, 256)
                }
            } else {
                if (database) {
                    texture = TextureManager::createTextureFromText(text, 512, 512)
                } else {
                    texture = TextureManager::createTextureFromTextAndImagePath(text, "logos/java.png", 512, 256)
                }
            }
        } else {
