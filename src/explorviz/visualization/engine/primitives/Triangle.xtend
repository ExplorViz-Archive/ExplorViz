...
class Triangle extends PrimitiveObject {
    ...
    protected int		  verticesMaxLength
    protected int		  textureCoordsMaxLength
    
    protected var started = false
    protected var alreadyClosed = false
    
    @Property float[]		     vertices
    @Property float[]		     textureCoords
    ...
    var	indexVertices     = 0
    var indexTextureCoords = 0
    int offsetStart
    
    new() {
        verticesMaxLength = verticesDimension * 3
        textureCoordsMaxLength = 2 * 3
        
        vertices = createFloatArray(verticesMaxLength)
        textureCoords = createFloatArray(textureCoordsMaxLength)
        color = createFloatArray(4)
        normal = createFloatArray(3)
    }
    ...
    def addTexturePoint(float s, float t) {
        if (!started) throw new IllegalArgumentException("not yet started")
        if (alreadyClosed) throw new IllegalArgumentException("already closed")
