...
public class ShaderInitializer {
	...
	public static void setShaderProgram(final WebGLProgram shaderProgram) {
		ShaderInitializer.shaderProgram = shaderProgram;
	};

	public static ShaderObject initShaders(final WebGLRenderingContext glContext) {
		ShaderInitializer.glContext = glContext;

		final WebGLShader fragmentShader = getShader(WebGLRenderingContext.FRAGMENT_SHADER,
				Resources.INSTANCE.fragmentShader().getText());
		final WebGLShader vertexShader = getShader(WebGLRenderingContext.VERTEX_SHADER,
		...
		final int vertexPositionAttribute = glContext.getAttribLocation(getShaderProgram(),
				"vertexPosition");
		glContext.enableVertexAttribArray(vertexPositionAttribute);

		final int textureCoordAttribute = glContext.getAttribLocation(getShaderProgram(),
				"texPosition");
		glContext.enableVertexAttribArray(textureCoordAttribute);

		final int vertexColorAttribute = glContext.getAttribLocation(getShaderProgram(),
				"vertexColor");
		glContext.enableVertexAttribArray(vertexColorAttribute);

		final int vertexNormalAttribute = glContext.getAttribLocation(getShaderProgram(),
				"vertexNormal");
		...
		final WebGLUniformLocation textureUniform = glContext.getUniformLocation(
				getShaderProgram(), "tex");

		final WebGLUniformLocation useTextureUniform = glContext.getUniformLocation(
	...
	private static WebGLShader getShader(final int type, final String source) {
	...
	private native static boolean isCompileShaderOkay(WebGLRenderingContext glContext,
			WebGLShader shader) /*-{
