...
class ObjectPicker {
	...
	private def static pickObject(int x, int y, int absoluteWidth, int absoluteHeight, EventType event) {
		if (hasEventHandlers(event) && WebGLStart::explorVizVisible) {
			val origin = unproject(x, y, 0, absoluteWidth, absoluteHeight - 100)
			var direction = unprojectDirection(0, 0, -100000f)
	...
	private def static unprojectDirection(float x, float y, float z) {
