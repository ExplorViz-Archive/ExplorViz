...
class Camera {
	...
	def static void moveXInVR(float distanceXInPercent) {
		cameraTranslate.x = distanceXInPercent * 6f * xPitch * (Math.abs(cameraTranslate.z) / 4f)
	}

	def static void moveYInVR(float distanceYInPercent) {
		cameraTranslate.y = distanceYInPercent * 6f * yPitch * (Math.abs(cameraTranslate.z) / 4f)
	}

	def static void moveZInVR(float distanceZInPercent) {
		cameraTranslate.z = distanceZInPercent * 4f * zPitch
	}
