...
public class Ray {
	...
	public boolean intersects(PrimitiveObject object) {
	...
	private boolean checkLine(Line line) {
	...
	private boolean checkPipe(Pipe pipe) {
	...
	public float getIntersectCoefficient(PrimitiveObject object) {
		if (object instanceof Box) {
	...
	private float getIntersectCoefficient(Box box) {
	...
	private float getIntersectCoefficient(Quad quad) {
		final List<Triangle> triangles = quad.getTriangles();
		return Math.min(getIntersectCoefficient(triangles.get(0)), getIntersectCoefficient(triangles.get(1)));
	}
	 ...
	 * @return
	 */
	private float getIntersectCoefficient(Triangle triangle) {
	...
	private float getIntersectCoefficient(Line line) {
		float minimum = Float.MAX_VALUE;
	...
	private float getIntersectCoefficient(Pipe pipe) {
		float minimum = Float.MAX_VALUE;
