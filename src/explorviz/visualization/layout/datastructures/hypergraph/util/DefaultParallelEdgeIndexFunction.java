...
public class DefaultParallelEdgeIndexFunction<V, E> implements EdgeIndexFunction<V, E> {
	...
	protected int getIndex(final Graph<V, E> graph, final E e, final V v, final V u) {
		final Collection<E> commonEdgeSet = new HashSet<E>(graph.getIncidentEdges(u));
		commonEdgeSet.retainAll(graph.getIncidentEdges(v));
		for (final Iterator<E> iterator = commonEdgeSet.iterator(); iterator.hasNext();) {
			final E edge = iterator.next();
			final Pair<V> ep = graph.getEndpoints(edge);
			final V first = ep.getFirst();
			final V second = ep.getSecond();
			// remove loops
