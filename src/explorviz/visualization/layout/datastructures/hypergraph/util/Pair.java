...
public final class Pair<T> implements Collection<T>, Serializable {
	 ...
	 *             != 2 elements.
	 */
	public Pair(final Collection<? extends T> values) {
		if (values == null) {
		...
		if (values.size() == 2) {
			...
			final Iterator<? extends T> iter = values.iterator();
			first = iter.next();
			second = iter.next();
		} else {
	...
	public boolean equals(final Object o) {
		...
		if (o instanceof Pair) {
			...
			final Object otherFirst = otherPair.getFirst();
			final Object otherSecond = otherPair.getSecond();
