...
public class GraphTraverser<T> {
	...
	private final GraphAdapter<T> graph;
	...
	private GraphAdapter<T> result = null;
	 ...
	 */
	public GraphTraverser(final GraphAdapter<T> graph) {
		this.graph = graph;
	}
	 ...
	 */
	public GraphAdapter<T> findPath(final T start, final T end, final Collection<T> banned) {
		searched.clear();
		searched.addAll(banned);
		result = new GraphAdapter<T>();
		goal = end;
	 ...
	 */
	public GraphAdapter<T> findCycle() {
		searched.clear();
		result = new GraphAdapter<T>();
	 ...
	 */
	private GraphAdapter<T> findCycle(final T v) {
		searched.add(v);
		for (final T n : graph.getNeighbors(v)) {
			if (n.equals(goal) && (result.countVertices() > 2)) {
				...
				result.addEdge(v, n);
				final GraphAdapter<T> completedCycle = findCycle(n);
	 ...
	 */
	public Set<GraphAdapter<T>> splitIntoPieces(final GraphAdapter<T> cycle) {
		searched.clear();
		final Set<GraphAdapter<T>> pieces = new HashSet<GraphAdapter<T>>();
	 ...
	 * node.
	 */
	private void makePiece(final GraphAdapter<T> cycle, final T v) {
