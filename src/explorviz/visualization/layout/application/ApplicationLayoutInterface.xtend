...
class ApplicationLayoutInterface {
	...
	def private static void calculateSize(Component component) {
		...
		for (child : component.children) {
			size = size + ((child.width+insetSpace/2f) * (child.depth+insetSpace/2f))
		}
		
		size = component.clazzes.size * ((clazzWidth+insetSpace/2f) * (clazzWidth+insetSpace/2f))
			
		
		
		
		component.width = Math.sqrt(size.doubleValue).floatValue + labelInsetSpace	
		component.depth = Math.sqrt(size.doubleValue).floatValue
	}
