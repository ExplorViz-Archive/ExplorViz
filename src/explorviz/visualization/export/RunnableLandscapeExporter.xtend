...
class RunnableLandscapeExporter {
	private static String JAVA_EXECUTABLE = '"C:\\Program Files\\Java\\jdk1.7.0_75\\jre\\bin\\java"'
	private static String ASPECTJ_WEAVER = 'aspectjweaver-1.8.5.jar'
	private static String MAIN_JAR = 'light-RPC-application.jar'
	private static int SERVER_RUNNING_FOR_SECONDS = 60

	private static int nextServerPort = 21000
	private static Map<Application, Integer> applicationToServerPort = new HashMap<Application, Integer>()
	private static Map<Application, Integer> applicationToIndexInResult = new HashMap<Application, Integer>()
	private static List<String> threadsToJoin = new ArrayList<String>()
...
//				applicationToIndexInResult.put(sourceApplication, applicationScriptIndex + paramsToAdd.length)
	...
	def static String createApplicationStageCode(Application applicationStage, int targetPort, int callsToServerCount,
		...
		applicationToServerPort.put(applicationStage, serverPort)
		val hostname = applicationStage.parent.name ?: applicationStage.parent.ipAddress

		val cmdApplicationStart = JAVA_EXECUTABLE + ' -javaagent:' + ASPECTJ_WEAVER +
			' -Dexplorviz.live_trace_processing.system_name="' + applicationStage.parent.parent.parent.name +
			'" -Dexplorviz.live_trace_processing.ip_address="' + applicationStage.parent.ipAddress +
			'" -Dexplorviz.live_trace_processing.host_name="' + hostname +
			'" -Dexplorviz.live_trace_processing.application_name="' + applicationStage.name + '" -jar ' + MAIN_JAR +
			' -serverPort ' + serverPort + ' -secondsToRunServer ' + SERVER_RUNNING_FOR_SECONDS +
			' -serverPortsToConnect ' + targetPort + ' -callsToServerCount ' + callsToServerCount

		val result = createApplicationCallCode(cmdApplicationStart, hostname, applicationStage.name, serverPort)

		applicationToIndexInResult.put(applicationStage, scriptIndex + result.length - '\')\nend\n\n'.length)
