...
import explorviz.plugin.capacitymanagement.cloud_control.common.*;
...
public class OpenStackCloudController implements ICloudController {
	...
	public OpenStackCloudController(final Configuration settings) {
	 ...
	 *             E.g. booterror.
	 */
	private String bootNewInstance(final String hostname, final ScalingGroup scalingGroup)
			throws Exception {
		LOG.info("Starting new instance of type " + scalingGroup.getApplicationFolder() + "...");
		final String bootCommand = "nova boot " + hostname + " --image " + scalingGroup.getImage()
	 ...
	 *             Copying failed.
	 */
	private void copyApplicationToInstance(final String privateIP, final ScalingGroup scalingGroup)
			throws Exception {
		LOG.info("Copying application '" + scalingGroup.getApplicationFolder() + "' to node "
				+ privateIP);
	...
	private void startApplicationOnInstance(final String privateIP, final ScalingGroup scalingGroup)
			throws Exception {
		LOG.info("Starting application script - " + scalingGroup.getStartApplicationScript()
				+ " - on node " + privateIP);
	 ...
	 *            Arraylist containing Node.
	 */
	private void waitForApplicationStart(final String privateIP, final ScalingGroup scalingGroup) {
		LOG.info("Waiting " + scalingGroup.getWaitTimeForApplicationStartInMillis()
