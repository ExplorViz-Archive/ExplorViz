 ...
 * '03)</i>, volume 2912 of LNCS, pp. 381-386. Springer, 2004.</li>
 * <li>Giuseppe di Battista, Peter Eades, Roberto Tamassia, Ioannis G. Tollis,
 * <i>Graph Drawing: Algorithms for the Visualization of Graphs</i>, Prentice
 * Hall, New Jersey, 1999 (Section 9.4, for cycle breaking in the hyperedge
 * segment graph)
 * </ul>
 * 
 * <p>
 * This is a generic implementation that can be applied to all four routing
 * directions. Usually, edges will be routed from west to east. However, with
 * northern and southern external ports, this changes: edges are routed from
 * south to north and north to south, respectively. To support these different
 ...
 * must be specified. Once that is done,
 * {@link #routeEdges(LGraph, List, int, List, double)} is called repeatedly to
...
public final class OrthogonalRoutingGenerator {
	...
	/**
	 * A routing direction strategy adapts the
	 * {@link OrthogonalRoutingGenerator} to different routing directions.
	...
	public interface IRoutingDirectionStrategy {
		 ...
		 *            the hyper edge.
		 * @param startPos
		 *            the position of the trunk of the first hyper edge between
		 *            the layers. This position, together with the current hyper
		 *            node's rank allows the calculation of the hyper node's
		 *            trunk's position.
		 */
	...
	public OrthogonalRoutingGenerator(final IRoutingDirectionStrategy.Strategy strategy,
		...
		switch (strategy) {
			case WEST_TO_EAST:
				...
				break;
			case NORTH_TO_SOUTH:
				routingStrategy = new NorthToSouthRoutingStrategy();
				break;
			case SOUTH_TO_NORTH:
				routingStrategy = new SouthToNorthRoutingStrategy();
				break;
			default:
	 ...
	 * @param portSide
	 *            side of the output ports for whose outgoing edges hypernodes
	 *            should be created.
	 * @param hyperNodes
	 ...
	 *            second hypernode
	 * @param minDiff
	 *            the minimal difference between horizontal line segments to
	 *            avoid a conflict
	 */
	...
	/**
	 * Breaks all cycles in the given hypernode structure by reversing or
	...
	private static void topologicalNumbering(final List<HyperNode> nodes) {
		// determine sources, targets, incoming count and outgoing count;
		// targets are only
		...
		/*
		 * If we stopped here, hyper nodes that don't have any horizontal
		 * segments pointing leftward would be ranked just like every other
		 * hyper node. This would move back edges too far away from their target
		 * node. To remedy that, we move all hyper nodes with horizontal
		 * segments only pointing rightwards as far right as possible.
		 */
		if (maxRank > -1) {
			// assign all target nodes with horzizontal segments pointing to the
			// right the
			// rightmost rank
			...
			// let all other segments with horizontal segments pointing
			// rightwards move as
	 ...
	 * @param vertical
	 *            {@code true} if the connecting segment is vertical,
	 *            {@code false} if it is horizontal
	 */
	private void addJunctionPointIfNecessary(final LEdge edge, final HyperNode hyperNode,
		...
		if (((p > hyperNode.start) && (p < hyperNode.end))
			...
			if (!createdJunctionPoints.contains(pos)) {
				...
				final KVector jpoint = new KVector(pos);
