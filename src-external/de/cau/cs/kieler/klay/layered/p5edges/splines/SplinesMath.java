...
public final class SplinesMath {
	...
	public static boolean isBetween(final int value, final int boundary0, final int boundary1) {
		return value < boundary0 ? boundary1 <= value : (value <= boundary1)
				|| (value == boundary0);
	}
	 ...
	 *         one is {@code (>= value)}.
	 */
	public static boolean isBetween(final double value, final double boundary0,
			final double boundary1) {
		if ((Math.abs(boundary0 - value) < EPSILON) || (Math.abs(boundary1 - value) < EPSILON)) {
			return true;
		}
		return (boundary0 - value) > EPSILON ? (value - boundary1) > EPSILON
