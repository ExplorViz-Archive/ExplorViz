...
public final class BKNodePlacer implements ILayoutPhase {
	...
	public void process(final LGraph layeredGraph, final IKielerProgressMonitor monitor) {
		...
		// Note that this condition can be true even if a balanced layout was
		// produced, so don't you
	...
	private void markConflicts(final LGraph layeredGraph) {
		for (int i = 1; i <= (layeredGraph.getLayers().size() - 2); i++) {
			...
			for (int l_1 = 0; l_1 < layerSize(layeredGraph, i + 1); l_1++) {
				...
				final LNode v_l_i = nodeByPosition(layeredGraph, i + 1, l_1);
				...
				if ((l_1 == ((layerSize(layeredGraph, i + 1)) - 1))
					...
					while (l <= l_1) {
						final LNode v_l = nodeByPosition(layeredGraph, i + 1, l);
	 ...
	 * 
	 * <p>
	 * These blocks, respectively the contained nodes, will be drawn straight
	...
	private void verticalAlignment(final LGraph layeredGraph, final BKAlignedLayout bal) {
		...
		if (bal.hdir == HDirection.BOTTOM) {
			layers = new ArrayList<Layer>(layeredGraph.getLayers());
		...
		for (final Layer layer : layers) {
			...
			if (bal.vdir == VDirection.RIGHT) {
				...
				// list of nodes.
				r = Integer.MAX_VALUE;
				nodes = new ArrayList<LNode>(layer.getNodes());
	 ...
	 * a block can be drawn straightly.
	 * </p>
	 * 
	 * <p>
	 * This phase is not included in the original algorithm and adds port and
	 * node size handling.
	 * </p>
	 * 
	 * @param layeredGraph
	...
	private void insideBlockShift(final LGraph layeredGraph, final BKAlignedLayout bal) {
		...
		for (final LNode root : blocks.keySet()) {
			...
			while ((next = bal.align.get(current)) != root) {
				...
				final double currentInnerShift = bal.innerShift.get(current) + difference;
				bal.innerShift.put(next, currentInnerShift);

				// Update the space required above and below the root node's top
				// left corner
				spaceAbove = Math.max(spaceAbove, next.getMargin().top - currentInnerShift);
	...
	private void horizontalCompaction(final LGraph layeredGraph, final BKAlignedLayout bal) {
		...
		// space
		// between them. It's important to traverse top-bottom or bottom-top
		// here too
		for (final Layer layer : layers) {
			for (final LNode v : layer.getNodes()) {
				...
				if (bal.vdir == VDirection.RIGHT) {
					if (v.equals(bal.root.get(v))
						...
						bal.y.put(v, bal.y.get(v) + bal.shift.get(bal.sink.get(v)));
					}
				} else {
					if (v.equals(bal.root.get(v))
	 ...
	 * moving the whole block up/downwards if there are blocks which already
	 * occupy the chosen position.
	 * </p>
	 * 
	 * @param v
	 *            The root node of the block
	 * @param bal
	 *            One of the four layouts which shall be used in this step
	 */
	private void placeBlock(final LNode v, final BKAlignedLayout bal) {
		...
		if (!bal.y.containsKey(v)) {
			...
			// Iterate through block and determine, where the block can be
			// placed
			do {
				// If the node is the top or bottom node of it's layer, it can
				// be, depending
				// on the current direction, placed safely since it is the first
				// to be placed
				if (((bal.vdir == VDirection.LEFT) && (w.getIndex() > 0))
						|| ((bal.vdir == VDirection.RIGHT) && (w.getIndex() < (w.getLayer()
								.getNodes().size() - 1)))) {

					// Get the node which is top/bottom to the node to be placed
					// to check,
					...
					// Check whether the comparison node is already placed,
					// place it if not
					placeBlock(u, bal);

					// Note that the two nodes and their blocks form a unit
					// called class in the
					// original algorithm. These are combinations of blocks
					// which play a role
					...
					// If two nodes aren't member of the same class, calculate
					// how the two classes
					...
					if (!bal.sink.get(v).equals(bal.sink.get(u))) {
						...
						if (bal.vdir == VDirection.RIGHT) {
							bal.shift.put(
									bal.sink.get(u),
									Math.max(bal.shift.get(bal.sink.get(u)),
											(bal.y.get(v) - bal.y.get(u)) + bal.blockSize.get(v)
													+ spacing));
						} else {
							bal.shift.put(
									bal.sink.get(u),
									Math.min(bal.shift.get(bal.sink.get(u)),
											bal.y.get(v) - bal.y.get(u) - bal.blockSize.get(u)
													- spacing));
						}
					} else {
						// If they are on the class, calculate a y position for
						// the current block,
						// using the information from the comparison node

						// Determine the available space in the current layer,
						// by taking node sizes,
						...
						if ((!(blockContainsNorthSouthDummy(bal, v) && blockContainsRegularNode(
								bal, u))
								&& !(blockContainsNorthSouthDummy(bal, u) && blockContainsRegularNode(
										bal, v)) && ((bal.blockSize.get(v) == 0.0) || (bal.blockSize
								.get(u) == 0.0)))) {
						...
						if (bal.vdir == VDirection.RIGHT) {
							bal.y.put(v, Math.min(bal.y.get(v),
									((bal.y.get(u) + bal.innerShift.get(x)) - x.getMargin().top)
											- spacing - wSize));

						} else {
							bal.y.put(v, Math.max(bal.y.get(v),
									(bal.y.get(u) + bal.innerShift.get(x) + x.getMargin().top)
											+ spacing + xSize));
						}
					}
	 ...
	 * used for determining the final position.
	 * </p>
	 * 
	 * @param layouts
	 ...
	 * @param position
	 *            The node's position, with 0 <= position <= layer.size - 1
	 * @return The node which is on the given position of the given layer or an
	 *         exception, if there is no node on the given position
	 */
	private LNode nodeByPosition(final LGraph layeredGraph, final int layer, final int position) {
	...
	private boolean incidentToInnerSegment(final LNode node, final int layer1, final int layer2) {
		if ((node.getProperty(Properties.NODE_TYPE) == NodeType.LONG_EDGE)
			...
			for (final LEdge edge : node.getIncomingEdges()) {
				if (((edge.getSource().getNode().getProperty(Properties.NODE_TYPE) == NodeType.LONG_EDGE) || (edge
						.getSource().getNode().getProperty(Properties.NODE_TYPE) == NodeType.COMPOUND_SIDE))
	 ...
	 * 
	 * An upper neighbor is a node in a previous layer which has an edge
	 ...
	 * 
	 * A lower neighbor is a node in a following layer which has an edge coming
	 * from the given node.
	 * 
	 * @param node
	 *            The node which might have neighbors
	...
	private HashMap<LNode, List<LNode>> getBlocks(final BKAlignedLayout bal) {
		...
		for (final LNode key : bal.root.keySet()) {
	 ...
	 *            The layout of which the blocks shall be found
	 * @param root
	 *            The root of the block to investigate
	...
	/**
	 * Checks whether any regular nodes are included in the block given by the
	 * root node.
	 * 
	 * @param bal
	 *            The layout of which the blocks shall be found
	...
	/**
	 * It is checked whether all nodes are placed in the correct order in their
	 * layers and do not overlap each other.
	 * 
	 * @param layeredGraph
	 *            the containing layered graph.
	 * @param bal
