...
public final class KimlUtil {
    ...
    /**
     * Ensures that each element contained in the given graph is attributed correctly for usage in
     * KIML. {@link KGraphElement}
     * 
     * @param graph
     *            the parent node of a graph
     */
    public static void validate(final KNode graph) {
	final KLayoutDataFactory layoutFactory = KLayoutDataFactory.eINSTANCE;
	...
	final Iterator<KNode> contentIter = Lists.newArrayList(graph).iterator();
	...
	while (contentIter.hasNext()) {
	    final EObject element = contentIter.next();
		...
		final KEdge edge = (KEdge) element;
    ...
    /**
     * Create a unique identifier for the given graph element. Note that this identifier is not
     * necessarily universally unique, since it uses the hash code, which usually covers only the
     * range of heap space addresses.
     * 
     * @param element
     *            a graph element
     */
    ...
    /**
     * Determines the port side for the given port from its relative position at its corresponding
     * node.
     * 
     * @param port
     *            port to analyze
    ...
    /**
     * Sets the size of a given node, depending on the minimal size, the number of ports on each
    ...
    /**
     * Converts the given relative point to an absolute location. The insets of the parent node are
    ...
    /**
     * Converts the given absolute point to a relative location. The insets of the parent node are
     * included in this calculation.
     * 
     * @param point
     *            an absolute point
     ...
     * KGraph by deserializing {@link PersistentEntry} tuples. Values are parsed using layout option
     * data obtained from the {@link LayoutDataService}. Options that cannot be resolved immediately
    ...
    /**
     * Determine the junction points of the given edge. This is done by comparing the bend points of
     * the given edge with the bend points of all other edges that are connected to the same source
     * port or the same target port.
     * 
     * @param edge
     *            an edge
    ...
    /**
     * Returns the signum function of the specified <tt>double</tt> value. The return value is -1 if
