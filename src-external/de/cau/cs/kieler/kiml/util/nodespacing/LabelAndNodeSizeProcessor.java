 ...
 * <dd>Port lists are properly sorted going clockwise, starting at the leftmost
 * northern port.</dd>
 * <dt>Postcondition:</dt>
...
public class LabelAndNodeSizeProcessor {
	 ...
	 * phase to be made available to later phases. This would probably be better
	 * kept in some kind of a context object, but is kept here for performance
	...
	/**
	 * Number of ports on the western side. Only used if port constraints are
	 ...
	 * for, the height includes the relevant port margins too. Only used if port
	...
	/**
	 * Number of ports on the northern side.Only used if port constraints are
	 * not {@link PortConstraints#FIXED_RATIO} or
	 * {@link PortConstraints#FIXED_POS}.
	 */
	...
	/**
	 * Number of ports on the southern side.Only used if port constraints are
	 * not {@link PortConstraints#FIXED_RATIO} or
	 * {@link PortConstraints#FIXED_POS}.
	 */
	...
	public void process(final GraphAdapter<?> layeredGraph) {
		...
		for (final NodeAdapter<?> node : layeredGraph.getNodes()) {
			 ...
			 * port margins are calculated. We currently only support one label
			 * per port.
			 */
	...
	private void calculateRequiredPortLabelSpace(final NodeAdapter<?> node) {
		...
		for (final PortAdapter<?> port : node.getPorts()) {
			switch (port.getSide()) {
				case WEST:
					requiredPortLabelSpace.left = Math.max(requiredPortLabelSpace.left,
							port.getMargin().right);
					break;
				case EAST:
					requiredPortLabelSpace.right = Math.max(requiredPortLabelSpace.right,
							port.getMargin().left);
					break;
				case NORTH:
					requiredPortLabelSpace.top = Math.max(requiredPortLabelSpace.top,
							port.getMargin().bottom);
					break;
				case SOUTH:
					requiredPortLabelSpace.bottom = Math.max(requiredPortLabelSpace.bottom,
							port.getMargin().top);
					break;
			}
		}
	...
	private void calculateRequiredNodeLabelSpace(final NodeAdapter<?> node,
		...
		// This method only sets insets if the node label is to be placed on the
		// inside
	...
	private void resizeNode(final NodeAdapter<?> node, final double portSpacing,
		...
		final EnumSet<SizeOptions> sizeOptions = node.getProperty(LayoutOptions.SIZE_OPTIONS);
		final PortConstraints portConstraints = node.getProperty(LayoutOptions.PORT_CONSTRAINTS);
		...
		// really; in this
		// case we will just assume the original node size to be the minumum for
		// ports
		...
		switch (portConstraints) {
			...
			case FIXED_ORDER:
				...
				minSizeForPorts = calculatePortSpaceRequirements(node, portSpacing,
						accountForLabels);
				break;
		...
		if (sizeConstraint.contains(SizeConstraint.PORTS)) {
			...
			if (accountForLabels) {
				...
				nodeSize.y = Math.max(nodeSize.y, requiredPortLabelSpace.top
						+ requiredPortLabelSpace.bottom + portSpacing);
			}
		}
		...
		if (sizeConstraint.contains(SizeConstraint.MINIMUM_SIZE)) {
			...
			if (sizeOptions.contains(SizeOptions.MINIMUM_SIZE_ACCOUNTS_FOR_INSETS)) {
				if (minWidth > 0) {
					nodeSize.x = Math.max(nodeSize.x, minWidth + requiredPortLabelSpace.left
							+ requiredPortLabelSpace.right);
				}

				if (minHeight > 0) {
					nodeSize.y = Math.max(nodeSize.y, minHeight + requiredPortLabelSpace.top
	 ...
	 *            the node's size before it was (possibly) resized.
	 */
			...
			switch (port.getSide()) {
				case WEST:
					port.getPosition().y = nodeSize.y
							* port.getProperty(LayoutOptions.PORT_RATIO_OR_POSITION);
	...
	private void placeNodeLabels(final NodeAdapter<?> node, final double labelSpacing) {
		...
		// Top left position where the node labels will start to be placed in a
		// vertical stack
		...
		if (nodeLabelPlacement.contains(NodeLabelPlacement.INSIDE)) {
			...
			// TODO: Outside placement doesn't take ports and port labels into
			// account yet.

			// Different placement logic depending on whether horizontal or
			// vertical placement
			// is prioritized
	...
	private void applyNodeLabelPositions(final NodeAdapter<?> node, final KVector startPosition,
		...
		// The horizontal alignment depends on where the labels are placed
		// exactly
		final EnumSet<NodeLabelPlacement> nodeLabelPlacement = node
				.getProperty(LayoutOptions.NODE_LABEL_PLACEMENT);
