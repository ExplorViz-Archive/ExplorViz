...
public final class KVector implements IDataObject, Cloneable {
	 ...
	 * @return {@code true} if the vectors are approximately equal,
	...
	/**
	 * Compares if this and the given vector are approximately equal. What
	 * <i>approximately</i> means is defined by the fuzzyness: for both x and y
	 * coordinate, the two vectors may only differ by at most the fuzzyness to
	 * still be considered equal.
	 * 
	 * @param other
	 *            the vector to compare this vector to.
	 * @param fuzzyness
	 *            the maximum difference per dimension that is still considered
	 *            equal.
	 * @return {@code true} if the vectors are approximately equal,
	 *         {@code false} otherwise.
	 */
	public boolean equalsFuzzily(final KVector other, final double fuzzyness) {
		return (Math.abs(x - other.x) <= fuzzyness) && (Math.abs(y - other.y) <= fuzzyness);
	}
