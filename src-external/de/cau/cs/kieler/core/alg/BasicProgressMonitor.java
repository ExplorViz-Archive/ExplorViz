...
/**
 * Base class for implementations of progress monitors. This class performs execution time
 * measurement, keeps track of the amount of completed work, and handles sub-tasks properly.
 * 
...
public class BasicProgressMonitor implements IKielerProgressMonitor {
    ...
    /**
     * the number of work units that will be consumed after completion of the currently active child
     * task.
     */
    ...
    /**
     * Creates a progress monitor with the given maximal number of hierarchy levels. If the number
     * is negative, the hierarchy levels are infinite. Otherwise progress is reported to parent
     * monitors only up to the specified number of levels.
     * 
     * @param themaxLevels
     *            the maximal number of hierarchy levels for which progress is reported
     */
     ...
     *            the maximal number of reported hierarchy levels, or -1 for infinite levels
     */
	...
	if (!closed) {
	    totalTime = (0L - startTime) * NANO_FACT;
    ...
    /**
     * Invoked when a sub-task is created, to be overridden by subclasses. This implementation
     * creates a new {@code BasicProgressMonitor} instance.
     * 
     * @param work
     *            amount of work that is completed in the current monitor instance when the sub-task
     *            ends
     * @param maxHierarchyLevels
     *            the maximal number of reported hierarchy levels for the parent progress monitor,
     *            or -1 for infinite levels
    ...
    /**
     * Invoked when work is done for this progress monitor, to be overridden by subclasses. This
